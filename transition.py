...
def get_transition_outcomes(game_env, state, action):
    """
    ...
    max_super_charge_outcome = max(game_env.super_charge_probs.keys())
    max_super_jump_outcome = max(game_env.super_jump_probs.keys())
    ...
    if (state.row == game_env.exit_row and state.col == game_env.exit_col and gems):
        #print("hh")
        return [(GameState(-1, -1, (1,)), 0, 1)]
    elif state.row == -1 or state.col == -1:
        #print("pp")
        return [(GameState(-1, -1, (1,)), 0, 1)]
    elif game_env.grid_data[state.row][state.col] == GameEnv.LAVA_TILE:
        #print("jj")
        return [(GameState(-1, -1, (1,)), 0, 1)]
